# DarkVeil — spec de integración (reactbits.dev)

Componente WebGL animado para fondos hero. Variante JavaScript + CSS.

## Dependencias

- `ogl` — librería WebGL liviana (renderer, program, mesh, etc.)
...
1. Install `ogl`.
2. Copy component source to `src/components/ui/DarkVeil.jsx` + `DarkVeil.css`.
3. Mount inside a parent with explicit dimensions and `position: relative`.
4. Tune props for portfolio palette — `hueShift` para verde accent.
