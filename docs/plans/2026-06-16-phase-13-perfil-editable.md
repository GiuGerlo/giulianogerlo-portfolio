...
- **2026-06-16**: Task 13 cerrada. Hero/Footer/Navbar/Contact leen `useSiteSettings` con fallback campo-a-campo (Hero: FALLBACK const; Footer/Navbar/Contact: a `socials.js` / CV estático). Página `/admin/sitio` (`Site.jsx`: Hero+Footer+CV(DocumentUpload)+redes, UPDATE eq id=1). Ruta lazy + nav AdminLayout refactorizada a array `NAV_ITEMS` (Proyectos/Perfil/Sitio). Tests: mock `useSiteSettings` en Hero/Footer/Navbar/Contact (fallback) + `Site.test.jsx` (load/submit). Suite 190/190 + lint + build OK (chunk Site 10.7kB lazy). Next: Task 14 (Skills).
- **2026-06-16**: Bug report CV (huérfano en Storage) → FALSO positivo: bucket `documents` vacío (solo `.emptyFolderPlaceholder`) y `cv_url=''` → `removeFile` funcionó; lo que se vio fue caché CDN. Sin cambios. Mejora CV: descarga forzada con `?download=cv-giuliano-gerlo.pdf` (cross-origin ignora atributo `download`) + `download="cv-giuliano-gerlo.pdf"`.
- **2026-06-16**: Task 14 cerrada. `Skills.jsx` runtime (`useSkillGroups` + `SKILL_ICONS`, fallback al data file). Página `/admin/skills` (`Skills.jsx` admin): CRUD inline con `useAdminList` + `IconPicker` + `ChipsEditor`, reorder ↑/↓, editar/borrar inline (patrón base para tasks 15-17). Ruta lazy `SkillsAdmin` + nav. Tests: `Skills.test` (section, mock hook) + `Skills.test` admin (5 tests CRUD/reorder, mock useAdminList). 195/195 + lint + build OK. Next: Task 15 (AI Integration).
- **2026-06-16**: Ajustes post-uso Task 14: (1) faltaba link "Skills" en `NAV_ITEMS` del AdminLayout → agregado. (2) `window.confirm` (feo) reemplazado por `ConfirmDialog.jsx` reusable (portal + backdrop + animación motion + Esc/focus/role=alertdialog); cableado en Skills admin Y ProjectForm (borrar proyecto). (3) `ChipsEditor` ahora reordena los chips con drag-and-drop (dnd-kit, distance constraint para que la X siga clickeable) → orden de tecnologías editable; beneficia también el stack de proyectos. 195/195 + lint + build OK.
- **2026-06-16**: Task 15 cerrada. `AISection.jsx` runtime (`useAiSkills`, fallback al data file; render lee `description ?? desc` por el rename DB). Página `/admin/ai` (`AiSkills.jsx`): CRUD inline (useAdminList) con `status` (select active/exploring) + Textarea description + ChipsEditor items + ConfirmDialog. Ruta lazy + nav "AI". Tests: AISection (mock hook) + AiSkills admin (3 tests). 198/198 + lint + build OK. Next: Task 16 (Experiencia).
- **2026-06-16**: Task 16 cerrada. `Experience.jsx` runtime (`useExperience`, fallback al data file; render `description ?? desc`). Página `/admin/experiencia` (`Experience.jsx` admin): CRUD inline con role/company/dateLabel/dateStart/dateEnd/description/current(checkbox)/projectSlug (select desde `useAdminProjects`) + ConfirmDialog. Ruta lazy + nav "Experiencia". Tests: section (mock hook) + admin (3, incluye select de proyecto). 201/201 + lint + build OK. Next: Task 17 (Educación + certs con DocumentUpload).
- **2026-06-16**: Task 17 cerrada. `Education.jsx` runtime (`useEducation`, fallback al data file; campos sin rename). Página `/admin/educacion` (`Education.jsx` admin): CRUD inline con dateLabel/title/org/status(select)/certUrl(`DocumentUpload` → bucket documents) + ConfirmDialog + link "Ver certificado". Ruta lazy + nav "Educación". Tests: section (mock hook) + admin (3, DocumentUpload stub). 204/204 + lint + build OK. Next: Task 18 (verificación end-to-end + cierre + limpieza data/*.js).
- **2026-06-16**: Task 18 cerrada → **PHASE 13 COMPLETA** (About + todo el sitio editable). Verificación: anon SELECT OK en las 5 tablas nuevas (site_settings 1, skill_groups 5, ai_skills 6, experience 6, education 4); anon INSERT/UPDATE → `permission denied` (RLS+grants); `get_advisors security` sin warnings nuevos (3 WARN pre-existentes ajenos); `pnpm lint` + `pnpm test:run` (204/204) + `pnpm build` OK; `git status` limpio. `data/*.js` se mantienen como FALLBACK (todos con consumidor) — ninguno huérfano. Todo el contenido del portfolio (Hero, About, Stack Técnico, AI Integration, Experiencia, Educación+certs, CV, Footer/redes) es 100% editable desde `/admin` sin redeploy.
