...
- **2026-06-11**: Task 12.10 cerrada (hardening de seguridad — verificación, sin cambios de código). **(1) RLS anon escritura**: INSERT/UPDATE/DELETE vía REST API con anon key → `401 permission denied for table projects` (code 42501). Defensa en doble capa: anon NUNCA tuvo grant de write (migration 0003 solo le dio SELECT), así que el denial dispara a nivel GRANT antes incluso de evaluar las policies RLS. **(2) Filtro published**: anon SELECT devuelve solo los 6 published; se insertó un draft temporal (`zzz-draft-rls-test`, published=false) vía service role, se confirmó que anon lo ve como `[]` (oculto por la policy "public read published") tanto por slug directo como en el listado, y se borró. **(3) Audit de bundle**: `pnpm build` + grep en `dist/` → la firma única de la `SUPABASE_SERVICE_ROLE_KEY` NO aparece en ningún archivo (0 matches); el `.js` ejecutable tiene 0 ocurrencias del término del rol; solo aparece como string en `index-*.js.map` (es código de la lib `@supabase/supabase-js`, no la key). **(4) CSP/headers**: `vercel.json` define X-Frame-Options/nosniff/Referrer-Policy/Permissions-Policy/HSTS pero NO `Content-Security-Policy`, así que el dominio Supabase no está bloqueado (fetches verificados funcionando). **Advisor de seguridad post-tests**: los mismos 3 warnings ajenos ya documentados en Task 12.2 (función `rls_auto_enable` SECURITY DEFINER managed por Supabase x2 + leaked-password N/A magic link) — cero regresión. **Hallazgos para futuro (no bloqueantes, fuera de scope 12.10)**: (a) los 18 sourcemaps se publican a prod por decisión documentada en `vite.config.js`; ahora exponen también el source del admin, pero la seguridad la impone RLS + allowlist server-side, no la ofuscación del cliente → aceptable; (b) opción de hardening futura: agregar header CSP con `connect-src` whitelisteando el dominio Supabase.
- **2026-06-11**: Hardening extra post-12.10 (a pedido del usuario): agregado header `Content-Security-Policy` en `vercel.json` (antes solo había X-Frame/nosniff/Referrer/Permissions/HSTS, sin CSP). Directivas: `default-src 'self'`; `script-src 'self' + challenges.cloudflare.com` (Turnstile; el bundle no tiene scripts inline, verificado en `dist/index.html`); `style-src 'self' 'unsafe-inline' + fonts.googleapis.com` (el `'unsafe-inline'` es necesario para los estilos inline de React/react-datepicker/dnd-kit); `font-src 'self' + fonts.gstatic.com data:`; `img-src 'self' data: https:`; `connect-src 'self' + dominio Supabase (https+wss) + challenges.cloudflare.com`; `frame-src challenges.cloudflare.com`; `object-src 'none'`; `base-uri 'self'`; `form-action 'self'`; `frame-ancestors 'none'`; `upgrade-insecure-requests`. **Verificación local antes de deploy**: como el header solo se aplica en Vercel (vite no lee `vercel.json`), se levantó un server temporal sirviendo `dist/` con el header exacto y se navegó con Playwright → cero violaciones CSP en consola; los 6 proyectos cargaron desde Supabase con imágenes (connect-src + img-src OK), fuentes y script de Turnstile cargaron (style/font/script-src OK). La verificación end-to-end en prod queda en Task 12.11.
...
### Task 12.10 — Hardening de seguridad ✅ (2026-06-11)

- [x] Verificar RLS con curl/Postman: anon NO puede insertar/update/delete. → `401 permission denied` (capa GRANT + RLS).
- [x] Verificar que anon solo ve `published=true`. → draft temporal oculto, confirmado y borrado.
- [x] Auditoría: revisar que `SUPABASE_SERVICE_ROLE_KEY` no esté en bundle. → firma de la key NO en `dist/`; término solo en sourcemap de la lib.
- [x] CSP/headers: confirmar que Supabase domain no rompe nada. → sin CSP header, nada bloqueado; fetches OK.
